...
But you probably struggled understanding and applying techniques like `pooling` and `dropout`.  
...
Basically, we take the network from an existing, well-performing model and we use every layer except the prediction layer, which we replace with our own fully-connected layer to meet our needs.  This approach is generally much faster and accurate.  
...
There are three elements to this Lab:
